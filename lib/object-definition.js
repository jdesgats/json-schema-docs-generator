...
ObjectDefinition.prototype.build = function(object) {
  ...
  } else if (_.isPlainObject(object.items)) {
    self.items = this.defineProperty(object.items);
  }
  ...
  _.each(required, function(prop) {
      // no not use `required` here as it is a JSON schema keyword
      self.allProps[prop].isRequired = true;
  });
